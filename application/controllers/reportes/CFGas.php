...
class CfGas extends CI_Controller {
    ...
    public function Cfgas() {
        ...
        $pdf->Cell( 97,4, utf8_decode( strtoupper( "DOCUMENTO TRIBUTARIO ELECTRÓNICO - FACTURA" ) ), 1, 0, 'C' );
        ...
        $pdf->Cell( 29, 8, utf8_decode( "Subtotal" ), 1, 0, 'C', true );
        ...
        $pdf->Cell( 7, 7, $respuesta[$i]->item, 'L', 0, 'C' );
        $pdf->Cell( 20, 7, $respuesta[$i]->cantidad, 'L', 0, 'C' );
        $pdf->Cell( 100, 7, utf8_decode( $respuesta[$i]->descripcion ), 'L', 0, 'C' );
        $pdf->Cell( 25, 7, utf8_decode( $respuesta[$i]->precioUnitario ), 'L', 0, 'C' );
       
        $pdf->Cell( 29, 7, utf8_decode( number_format($respuesta[$i]->subtotal, 2, '.', '' ) ), 'LR', 0, 'C' );
        $pdf->Ln( 7 );
       ...
       $pdf->SetFont( 'Arial', 'B', 7 );
        ...
        # Impuestos & totales #
        $pdf->Cell( 127, 7, utf8_decode( '' ), 'T', 0, 'C' );
       
        $pdf->Cell( 26, 7, utf8_decode( "SUBTOTAL" ), 'T', 0, 'J' );
        $pdf->Cell( 32, 7, utf8_decode( number_format( $totalGravado , 2, '.', '' ) ), 'T', 0, 'C' );

        $pdf->Ln( 5 );

        $pdf->Cell( 130, 7, utf8_decode( '' ), '', 0, 'C' );
        
        $pdf->Cell( 22, 7, utf8_decode( "IVA (".($iva13*100)."%)" ), '', 0, 'J' );
        $pdf->Cell( 32, 7, utf8_decode( $ivaResumen ), '', 0, 'C' );
        $pdf->Ln( 5 );

        $pdf->Cell( 130, 7, utf8_decode( '' ), '', 0, 'C' );
            
        $pdf->Cell( 22, 7, utf8_decode( "FOVIAL $".$fovial."" ), '', 0, 'J' );
        $pdf->Cell( 32, 7, utf8_decode( $fovialRet ), '', 0, 'C' );
        $pdf->Ln( 5 );

        $pdf->Cell( 130, 7, utf8_decode( '' ), '', 0, 'C' );
            
        $pdf->Cell( 22, 7, utf8_decode("COTRANS $".$cotrans.""), '', 0, 'J' );
        $pdf->Cell( 32, 7, utf8_decode( $cotransRet ), '', 0, 'C' );

        $pdf->Ln( 5 );
        $pdf->Cell( 130, 7, utf8_decode( '' ), '', 0, 'C' );
            
        $pdf->Cell( 22, 7, utf8_decode( "TOTAL A PAGAR" ), 'T', 0, 'J' );
        $pdf->Cell( 32, 7, utf8_decode( number_format( $totalGravado+$cotransRet+$fovialRet , 2, '.', '' )), 'T', 0, 'C' );
