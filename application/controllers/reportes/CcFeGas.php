...
class CcfeGas extends CI_Controller {
    ...
    public function Ccfegas() {
       ...
       $numeroControl = $this->input->post( "numeroControl" );
        ...
        $pdf->Cell( 97,4, utf8_decode( strtoupper( "DOCUMENTO TRIBUTARIO ELECTRÓNICO - CREDITO FISCAL" ) ), 1, 0, 'C' );
 ...
 # Tabla de productos #
        ...
        $pdf->Cell( 32, 7, utf8_decode( number_format($respuesta[$i]->subtotal, 2, '.', '' ) ), 'LR', 0, 'C' );
        $pdf->Ln( 7 );
       ...
       $pdf->SetFont( 'Arial', 'B', 7 );
        ...
        # Impuestos & totales #
        $pdf->Cell( 130, 7, utf8_decode( '' ), 'T', 0, 'C' );
        ...
        $pdf->Cell( 22, 7, utf8_decode( "FOVIAL $".$fovial."" ), '', 0, 'C' );
        $pdf->Cell( 32, 7, utf8_decode( $fovialRet ), '', 0, 'C' );
        $pdf->Ln( 5 );

        $pdf->Cell( 130, 7, utf8_decode( '' ), '', 0, 'C' );
            
        $pdf->Cell( 22, 7, utf8_decode("COTRANS $".$cotrans.""), '', 0, 'C' );
        ...
        $pdf->Cell( 32, 7, utf8_decode( number_format( $totalGravado+$ivaResumen+$fovialRet+$cotransRet , 2, '.', '' )), 'T', 0, 'C' );
